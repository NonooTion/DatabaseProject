<template>
         <!-- 添加对话框 -->
         <el-dialog v-model="addDialogVisiable" style="width: 50%; height: 80%; margin-top: 5%;" title="请输入有关信息">
        <el-form labelPosition="top" ref="addForm" :model="addForm" :rules="rules">
          <el-form-item label="课程名称:" prop="className">
            <el-input v-model="addForm.className" placeholder="请输入课程名称"></el-input>
          </el-form-item>
          ...
          <el-form-item label="课程简介:" prop="description">
            <el-input type="textarea" v-model="addForm.description" placeholder="请输入课程简介"></el-input>
          </el-form-item>
          <el-form-item label="开始日期:" prop="startDate">
            <el-date-picker v-model="addForm.startDate" value-format="YYYY-MM-DD"  placeholder="请选择开始日期" :disabled-date="disabledDate"></el-date-picker>
          </el-form-item>
          <el-form-item label="结束日期:" prop="endDate">
            <el-date-picker v-model="addForm.endDate" value-format="YYYY-MM-DD"  placeholder="请选择结束日期" :disabled-date="disabledDate"></el-date-picker>
          </el-form-item>
        ...
        <el-form labelPosition="top" ref="editForm" :model="editForm" :rules="rules">
          <el-form-item label="课程名称:" prop="className">
            <el-input v-model="editForm.className" placeholder="请输入课程名称"></el-input>
          </el-form-item>
          <el-form-item label="教练:" prop="coachId" >
          ...
          <el-form-item label="课程简介:" prop="description">
            <el-input type="textarea" v-model="editForm.description" placeholder="请输入课程简介"></el-input>
          </el-form-item>
          <el-form-item label="开始日期:" prop="startDate">
            <el-date-picker v-model="editForm.startDate" value-format="YYYY-MM-DD"  placeholder="请选择开始日期" :disabled-date="disabledDate"></el-date-picker>
          </el-form-item>
          <el-form-item label="结束日期:" prop="endDate">
            <el-date-picker v-model="editForm.endDate" value-format="YYYY-MM-DD"  placeholder="请选择结束日期" :disabled-date="disabledDate"></el-date-picker>
          </el-form-item>
          <el-form-item label="开始时间" prop="startTime">
            <el-time-select v-model="editForm.startTime"  start="07:00" end="22:00"step='01:00' placeholder="请选择开始时间" style="width: 50%;"/>
          </el-form-item>
          <el-form-item label="结束时间" prop="endTime">
            <el-time-select v-model="editForm.endTime"  start="07:00"end="22:00"step='01:00' placeholder="请选择结束时间" style="width: 50%;"/>
          </el-form-item>
          <el-form-item>
            <el-form-item label="价格:" prop="price">
  ...
  <el-container>
    <el-header style="padding:0px;">
        <el-form class="queryForm" :inline="true">
          <el-form-item label="课程名称:" >
              <el-input placeholder="请输入课程名称" v-model="queryData.className" ></el-input>
          </el-form-item>
          <el-form-item label="教练名称:" style="width: 500px;">
            <el-select v-model="queryData.coachId" placeholder="请选择教练">
                 <el-option label="请选择教练" value=""></el-option>
                <el-option v-for="item in coaches" :key="item.userId" :label="item.userId + '-' + item.name" :value="item.userId">
                </el-option>
            </el-select>
          </el-form-item>
    ...
    <el-main style="padding: 0px;">
     <el-table :data="groupClassData" stripe border fit highlight-current-row class="table" ref="tableRef">
...
export default{
  data(){
      return{
         ...
         rules:{
             ...
             className:[{required:true,message:"请输入团课名称",trigger:"blur"}],
             description:[{required:true,message:"请输入课程简介",trigger:"blur"}],
             endDate:[{required:true,message:"请选择开始日期",trigger:"blur"}],
             startDate:[{required:true,message:"请选择结束日期",trigger:"blur"}],
  ...
  methods:{
      toggleFullDescription(index, row) { 
        this.groupClassData[index] = { ...this.groupClassData[index], showFullDescription: !row.showFullDescription }
      },   
        upload(file)
      {
        console.log(file);
        let url=this.groupClassUrl+"/upload"
        let formData = new FormData();  
        formData.append('file', file.file); 
        formData.append('classId',file.data.classId),
   ...
   handleEdit(row){
     ...
     this.editForm.startDate=this.$moment(new Date(row.startDate)).format('YYYY-MM-DD')
     this.editForm.endDate=this.$moment(new Date(row.endDate)).format('YYYY-MM-DD')
   ...
   editSubmit()
   {
         ...
         this.$axios.get(url,{params:params}).then(
             (res)=>{
                 if(res.data.code===this.$code.SELECT_SUCCESS){
                     this.groupClassData=res.data.data
     ...
     this.$confirm(
       ...
       let url=this.groupClassUrl;
       let params={classId: row.classId};
